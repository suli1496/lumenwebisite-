/* ============================================================
   ...
   3) cinematische canvas-vuurtoren in de hero (volgt de muis,
      lichtdeeltjes, pulserende beacon) — tekent in de kleuren van
      het actieve thema, zodat dag en nacht hetzelfde sjabloon delen
...
function initHeroScene() {
  ...
  // bron van het licht (de lantaarn) — rechts in beeld, variant A
  ...
  function draw() {
    t += .016;
    // lucht-gradiënt (subtiel in dag, cinematisch in nacht)
    var sky = x.createLinearGradient(0, 0, 0, H);
    sky.addColorStop(0, col.top); sky.addColorStop(1, col.bottom);
    x.fillStyle = sky; x.fillRect(0, 0, W, H);

    var s = src();
    // doelhoek van de bundel: naar de muis, of rustig zwaaiend
    var ang;
    ...
    x.fillRect(-40, 92, 80, 4);          // grond
    x.fillRect(-15, 62, 30, 32);         // onderromp
    x.fillRect(-11, 34, 22, 28);         // bovenromp
    x.fillRect(-13, 24, 26, 10);         // galerij
    // lantaarn-gloed
